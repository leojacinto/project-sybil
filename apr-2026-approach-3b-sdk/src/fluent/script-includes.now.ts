...
TravelPolicyUtil.prototype = {
  ...
  getPolicyForCountry: function(country) {
    ...
    gr = new GlideRecord('x_snc_apr_trv_policy');
    gr.addQuery('x_snc_apr_trv_applies_to_country', '*');
    gr.addQuery('x_snc_apr_trv_active', true);
    gr.query();
    if (gr.next()) return gr;
    return null;
  ...
  isWithinPolicy: function(request) {
    var policy = this.getPolicyForCountry(request.x_snc_apr_trv_destination_country + '');
    if (!policy) return true;
    return parseFloat(request.x_snc_apr_trv_estimated_cost) <= parseFloat(policy.x_snc_apr_trv_finance_threshold);
  },
...
TravelDelegationUtil.prototype = {
  ...
  getActiveDelegate: function(managerSysId) {
    ...
    gr.addQuery('x_snc_apr_trv_delegator', managerSysId);
    gr.addQuery('x_snc_apr_trv_active', true);
    var today = new GlideDate();
    gr.addQuery('x_snc_apr_trv_start_date', '<=', today);
    gr.addQuery('x_snc_apr_trv_end_date', '>=', today);
    gr.query();
    if (gr.next()) return gr.x_snc_apr_trv_delegate + '';
    return null;
  },

  isDelegationActive: function(delegator, date) {
    var gr = new GlideRecord('x_snc_apr_trv_delegation');
    gr.addQuery('x_snc_apr_trv_delegator', delegator);
    gr.addQuery('x_snc_apr_trv_active', true);
    gr.addQuery('x_snc_apr_trv_start_date', '<=', date);
    gr.addQuery('x_snc_apr_trv_end_date', '>=', date);
    gr.query();
    return gr.hasNext();
  },
...
TravelCostCalculator.prototype = Object.extendsObject(global.AbstractAjaxProcessor, {
  ...
  getEstimatedDailyRate: function() {
    ...
    if (!policy) return '0';
    if (expenseType == 'hotel') return policy.x_snc_apr_trv_max_daily_hotel + '';
    if (expenseType == 'meals') return policy.x_snc_apr_trv_max_daily_meals + '';
    return '0';
  },
  ...
  getPolicyLimit: function() {
    ...
    return policy.x_snc_apr_trv_finance_threshold + '';
  },
